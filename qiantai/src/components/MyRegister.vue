...
export default {
  ...
  data() {
    ...
    let validateName = (rule, value, callback) => {
      ...
      const userNameRule = /^[a-zA-Z0-9_]{0,15}$/;
      if (!userNameRule.test(value)) {
        return callback(new Error("长度1-16之间,允许字母数字下划线"));
      }
    };
    // 密码的校验方法
    let validatePass = (rule, value, callback) => {
      ...
      const passwordRule = /^[a-zA-Z0-9_]{0,17}$/;
      if (passwordRule.test(value)) {
        this.$refs.ruleForm.validateField("checkPass");
        return callback();
      } else {
        return callback(
          new Error("长度0-18之间,允许字母数字和下划线")
        );
      }
    };
    // 确认密码的校验方法
  ...
  methods: {
    Register() {
      ...
      // 通过element自定义表单校验规则，校验用户输入的用户信息
          ...
          this.$axios.post("/api/front/registeryonghuxinxi", {
