    ...
    <div class="order-content">
      <div class="content">
        ...
        <ul>
          ...
          <li class="header">
            <div class='pro-img'> 网点名称</div>
            <div class='pro-img'> 网点联系方式</div>
            <div class='pro-img'> 网点位置</div>
            <div class='pro-img'> 类型名称</div>
            <div class='pro-img1'> 预约日期</div>
          </li>

          <li class="product-list" v-for="item in yuyuexinxi" :key="item.id">
            <router-link :to="`/yuyuexinxidetail/`+item.id">
              <div class='pro-img'>{{ item.wangdianmingcheng }}</div>
              <div class='pro-img'>{{ item.wangdianlianxifangshi }}</div>
              <div class='pro-img'>{{ item.wangdianweizhi }}</div>
              <div class='pro-img'>{{ item.leixingmingcheng }}</div>
              <div class='pro-img1'>{{ removeLastZero(item.yuyueriqi) }}</div>
...
export default {
  ...
  created() {
    ...
    this.getyuyuexinxi(this.formInline);
  },
  activated() {
  ...
  methods: {
    ...
    // 向后端请求分类列表数据
    getyuyuexinxi(parameter) {
      this.$axios
          .post("/api/front/yuyuexinxi", {...parameter})
          .then(res => {
            debugger
            this.yuyuexinxi = res.data.data.list;
