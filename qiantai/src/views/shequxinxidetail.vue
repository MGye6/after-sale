      ...
      <div class="content">
        <table width="100%" border="0" align="center" cellpadding="3" cellspacing="1"
          ...
          <tr>
            ...
            <td width='39%' height='60px;'>{{ shequxinxidetail.wangdianmingcheng }}</td>
            <td width='11%' height='60px;'>网点联系方式：</td>
            <td width='39%' height='60px;'>{{ shequxinxidetail.wangdianlianxifangshi }}</td>
          </tr>
          <tr>
            <td width='11%' height='60px;'>网点位置：</td>
            <td width='39%' height='60px;'>{{ shequxinxidetail.wangdianweizhi }}</td>
            <td width='11%' height='60px;'>网点类型：</td>
            <td width='39%' height='60px;'>{{ shequxinxidetail.wangdianleixing }}</td>
          </tr>
          <tr>
            <td width='11%' height='60px;'>备件名称：</td>
            <td width='39%' height='60px;'>{{ shequxinxidetail.beijianmingcheng }}</td>
            <td width='11%' height='60px;'>花费金额：</td>
            <td width='39%' height='60px;'>{{ shequxinxidetail.huafeijine }}</td>
          </tr>
          <tr>
            <td width='11%' height='60px;'>用户名：</td>
            <td width='39%' height='60px;'>{{ shequxinxidetail.yonghuming }}</td>
          </tr>
        </table>
        <div class="button">
          <!--dshoucang1-->
...
export default {
  ...
  created() {
    this.shequxinxiID = this.$route.params.id;
  ...
  methods: {
    getpinglun() {
      this.$axios
          .post("/api/front/pinglun", {biao: "shequxinxi", shujuid: this.id})
          .then(res => {
    ...
    handleEdit: function () {
      ...
      this.title = '添加评论'
      this.editForm.biao = "shequxinxi";
      this.editForm.shujuid = this.id;
    ...
    getDetails(val) {
      this.$axios
          .get("/api/front/getshequxinxidetail/" + val,)
          .then(res => {
            debugger
            this.shequxinxidetail = res.data.data;
          })
          .catch(err => {
            return Promise.reject(err);
          });
    ...
    addCollect() {
      ...
      this.$axios
          .post("/api/front/addCollect", {
            ...
            shangpinxinxiID: this.shequxinxiID
          })
          .then(res => {
            if (res.data.code == "0") {
