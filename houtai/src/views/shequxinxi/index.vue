<template>
  <div>
    <el-form :inline="true" :model="formInline" class="user-search">
      ...
      <el-form-item label="">
        <el-input size="small" v-model="formInline.wangdianlianxifangshi" placeholder="输入网点联系方式"></el-input>
      </el-form-item>
      <el-form-item label="">
        <el-input size="small" v-model="formInline.wangdianleixing" placeholder="输入网点类型"></el-input>
      </el-form-item>
      ...
      <el-form-item>
        <el-upload class="upload-demo" style="float: left; padding-right: 10px;"
                   action='http://localhost:9999/shequxinxi/upload' :headers='headers' :show-file-list='false'
    ...
    <el-table size="small" :data="listData" highlight-current-row v-loading="loading" border
      ...
      <el-table-column sortable prop="wangdianleixing" label="网点类型"></el-table-column>
      <el-table-column sortable prop="beijianmingcheng" label="备件名称"></el-table-column>
      <el-table-column sortable prop="huafeijine" label="花费金额"></el-table-column>
      ...
      <el-table-column align="center" label="操作" min-width="160">
        <template slot-scope="scope">
          <el-button size="mini" type="danger" @click="deleteShequxinxi(scope.$index, scope.row)"
                     v-if="userInfo.roles[0] == 'guanliyuan'  || (userInfo.roles[0] == 'yonghu' && userInfo.yonghuming == scope.row.yonghuming)">删除
          </el-button>
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click="closeDialog">
      <el-form label-width="120px" :model="editForm" :rules="rules" ref="editForm">
        ...
        <el-form-item label="网点类型" prop="wangdianleixing">
          <el-input size="small" v-model="editForm.wangdianleixing" auto-complete="off" placeholder="请输入网点类型"
                    style='width:50%' disabled></el-input>
        </el-form-item>
        <el-form-item label="备件名称" prop="beijianmingcheng">
          <el-input size="small" v-model="editForm.beijianmingcheng" auto-complete="off" placeholder="请输入备件名称"
                    style='width:50%' disabled></el-input>
        </el-form-item>
        <el-form-item label="花费金额" prop="huafeijine">
          <el-input size="small" v-model="editForm.huafeijine" auto-complete="off" placeholder="请输入花费金额"
                    style='width:50%' disabled></el-input>
        </el-form-item>
...
import {
  ...
  shequxinxiDelete,
  shequxinxiEdit,
  shequxinxiDeleteList
} from '@/api/shequxinxi/shequxinxiApi';
...
export default {
  ...
  methods: {
    ...
    //zxhifuff
    handleShenheList() {
      const shequxinxis = this.shequxinxis;
      if (shequxinxis.length == 0) {
        this.$message({
          type: 'error',
          message: '请至少选择一项'
        })
        ...
        this.$confirm('确定要批量审核吗?', '信息', {
          ...
          shequxinxis.forEach(item => {
            let data = {...item, issh: 1}
            shequxinxiEdit(data).then(res => {
            })
          })
          this.getdata(this.formInline)
          this.$message({
    ...
    handleDetail(index, row) {
      ...
      w.location.href = 'http://localhost:8080/#/shequxinxidetail/' + row.id;
    },
    //liangbuedit
    // 编辑、增加页面保存方法
    submitForm(editData) {
      ...
      this.$refs[editData].validate(valid => {
        if (valid) {
          if (this.editForm.id) {
            shequxinxiEdit(this.editForm).then(res => {
    ...
    // 删除公司
    deleteShequxinxi(index, row) {
      this.$confirm('确定要删除吗?', '信息', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        shequxinxiDelete(row.id).then(res => {
          if (res.code == '0') {
            this.$message({
              type: 'success',
              message: '社区信息已删除!'
    ...
    handleDeleteList() {
      ...
      if (shequxinxis.length == 0) {
        ...
        this.$confirm('确定要批量删除吗?', '信息', {
          ...
          shequxinxiDeleteList(shequxinxis).then(res => {
            if (res.code == '0') {
              this.$message({
                type: 'success',
                message: '批量删除成功!'
