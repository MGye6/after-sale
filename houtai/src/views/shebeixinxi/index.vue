<template>
  <div>
    <el-form :inline="true" :model="formInline" class="user-search">
      ...
      <el-form-item>
        ...
        <el-button size="small" type="primary" icon="el-icon-plus" @click="handleEdit()"
                   v-if="userInfo.roles[0] == 'yonghu'">添加
        </el-button>
    ...
    <el-table size="small" :data="listData" highlight-current-row v-loading="loading" border
      ...
      <el-table-column align='center' prop='shebeitupian' label='设备图片' width='120'>
        <template slot-scope='scope'>
          <el-image :src='scope.row.shebeitupian' style='height:70px' v-if="scope.row.shebeitupian"/>
      ...
      <el-table-column sortable prop="shebeicanshu" label="设备参数"></el-table-column>
      ...
      <el-table-column align="center" label="操作" min-width="160">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="danger" @click="deleteShebeixinxi(scope.$index, scope.row)"
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click="closeDialog">
      <el-form label-width="120px" :model="editForm" :rules="rules" ref="editForm">
        <el-form-item label="设备名称" prop="shebeimingcheng">
          <el-input size="small" v-model="editForm.shebeimingcheng" auto-complete="off" placeholder="请输入设备名称"
        ...
        <el-form-item label='设备图片' prop='shebeitupian'>
          <el-upload class='avatar-uploader' action='http://localhost:9999/files/upload' :headers='headers'
                     :show-file-list='false' :on-success='handleShebeitupianSuccess'
                     :before-upload='beforeShebeitupianUpload'><img v-if='editForm.shebeitupian'
                                                                    :src='editForm.shebeitupian' class='avatar'><i
        ...
        <el-form-item label="防伪码" prop="fangweima">
          <el-input size="small" v-model="editForm.fangweima" auto-complete="off" placeholder="请输入防伪码"
                    style='width:50%'></el-input>
        </el-form-item>
        <el-form-item label="设备参数" prop="shebeicanshu">
          <el-input size="small" v-model="editForm.shebeicanshu" auto-complete="off" placeholder="请输入设备参数"
                    style='width:50%'></el-input>
        </el-form-item>
...
import {
  ...
  shebeixinxiDelete,
  shebeixinxiEdit,
  shebeixinxiDeleteList
} from '@/api/shebeixinxi/shebeixinxiApi';
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        shebeitupian: [{required: true, message: '请输入设备图片', trigger: 'blur'},
        ],
  ...
  methods: {
    ...
    //zxhifuff
    handleShenheList() {
      const shebeixinxis = this.shebeixinxis;
      if (shebeixinxis.length == 0) {
        this.$message({
          type: 'error',
          message: '请至少选择一项'
        })
        ...
        this.$confirm('确定要批量审核吗?', '信息', {
          ...
          shebeixinxis.forEach(item => {
            let data = {...item, issh: 1}
            shebeixinxiEdit(data).then(res => {
            })
          })
          this.getdata(this.formInline)
          this.$message({
    ...
    daochu() {
      axios.get('/shebeixinxi/getExcel', {
        responseType: 'blob',
    ...
    handleDetail(index, row) {
      ...
      w.location.href = 'http://localhost:8080/#/shebeixinxidetail/' + row.id;
    },
    //liangbuedit
    // 编辑、增加页面保存方法
    submitForm(editData) {
      ...
      this.$refs[editData].validate(valid => {
        if (valid) {
          if (this.editForm.id) {
            shebeixinxiEdit(this.editForm).then(res => {
            ...
            shebeixinxiSave(this.editForm).then(res => {
              this.editFormVisible = false
              this.loading = false
              if (res.code == '0') {
    ...
    // 删除公司
    deleteShebeixinxi(index, row) {
      this.$confirm('确定要删除吗?', '信息', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        shebeixinxiDelete(row.id).then(res => {
          if (res.code == '0') {
            this.$message({
              type: 'success',
              message: '设备信息已删除!'
    ...
    handleDeleteList() {
      ...
      if (shebeixinxis.length == 0) {
        ...
        this.$confirm('确定要批量删除吗?', '信息', {
          ...
          shebeixinxiDeleteList(shebeixinxis).then(res => {
            if (res.code == '0') {
              this.$message({
                type: 'success',
                message: '批量删除成功!'
    ...
    handleShebeitupianSuccess(res, file) {
      if (res.code == "0") {
        this.editForm.shebeitupian = "/files/download/" + res.data.id
        this.$forceUpdate();
      }
    },
    beforeShebeitupianUpload(file) {
